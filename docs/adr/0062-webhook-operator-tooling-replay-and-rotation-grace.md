...
## Addendum: the failure ladder

Recording attempts without reacting to a failure streak left `enabled` a purely manual flag, so an endpoint whose receiver rotted away retried forever. The ladder closes that: `webhook_endpoints.consecutive_failures` counts consecutive failed delivery attempts — every recorded failure (retryable, permanent, or dead-lettered) climbs it, a delivered attempt resets it to zero, and the counter moves in the same D1 batch as the delivery row, with the streak returned to the consumer. The thresholds live in the delivery plan (`WEBHOOK_FAILURE_NOTIFY_RUNGS`, `WEBHOOK_FAILURE_AUTO_DISABLE_AT`): at 5, 10, and 15 consecutive failures the workspace owners get an owner-targeted in-app Notification (plus its email per their channel preference) of the existing `webhook.delivery_failed` kind — the ladder rides the kind webhook failures already own rather than minting a second vocabulary entry beside it — and at 20 the consumer calls `WebhookEndpoints.autoDisableEndpoint`, which sets `enabled = false` and batches a `webhook_endpoint.auto_disabled` audit event with the write. The disable rung is a floor, not an exact match, so an endpoint re-enabled mid-streak without a success disables again on its next failure; re-enabling stays the operator's one path, `update { enabled: true }`.
