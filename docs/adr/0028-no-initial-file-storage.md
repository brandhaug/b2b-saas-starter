...
One workflow has since needed object storage: the Workspace Export (ADR 0055) writes its gzipped JSON artifacts to a single, env-gated R2 bucket with a seven-day lifecycle rule. That bucket holds export artifacts only — no user uploads, no general file storage — and the starter still ships no upload surface; the rest of this decision stands.
