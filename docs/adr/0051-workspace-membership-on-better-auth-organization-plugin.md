...
Role changes and member removal invalidate private conversation access before
the plugin write. Failed archive cleanup or host notification therefore leaves
the membership mutation retryable. A subsequent plugin refusal may conservatively
invalidate an unchanged member's archives and active answers. Existing D1
membership triggers also advance policy and run revisions, protecting direct
plugin writes; they do not replace archive cleanup or immediate host notification.
