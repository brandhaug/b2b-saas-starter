...
Amended (issue #98): ban/unban and cross-workspace role changes now ship in `/admin` through the `PlatformUserAdmin` capability, audited as `system_admin.*` events. Impersonation remains unexposed per the constraint above.
