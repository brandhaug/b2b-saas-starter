...
| SQLite Durable Object conversations, accepted prompts, attempts and replay storage                    | [Durable Object data security](https://developers.cloudflare.com/durable-objects/reference/data-security/) documents automatic AES-256 disk encryption with Cloudflare-managed keys and TLS for Worker/internal/API transfers. It does not state an internal TLS minimum or separately establish every recovery-copy guarantee.       | Inventory the stage namespace, object class and all web/API/background bindings. Confirm recovery-copy coverage, location, deletion fences and replay cleanup; retain provider evidence for unobservable binding hops. D1-only backups do not contain transcripts. See [conversation recovery](backup-recovery.md). |
...
The web worker permits credential-free HTTPS `GET /api/auth/jwks` requests when
Cloudflare supplies an empty TLS version and ASN 13335. This matches observed
Worker-origin public-key fetches, which otherwise fail before member OAuth token
verification. [ASN 13335 also covers other Cloudflare traffic](https://developers.cloudflare.com/analytics/faq/other-faqs/);
it does not prove an internal connection or a TLS minimum. This bounded public-metadata allowance
rejects cookies, Authorization headers, query strings, HTTP and explicit old TLS.
All other routes retain the general transport gate. Treat this hop's exact TLS
version as unverified; OAuth clients still fetch keys over certificate-verified
HTTPS and verify token signatures, issuer and resource audience.
