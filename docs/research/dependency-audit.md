...
| Mermaid                         | Checked-in SVG or Mermaid Tiny                         | One static diagram makes SVG plausible, but removing Mermaid retires the documented MDX diagram-authoring feature. Retain that starter capability; do not add another renderer.                                                              |
...
| Sentry/Effect OTLP              | Raw vendor HTTP calls or native Workers OTLP           | Keep these SDK integrations; PostHog capture is already a plain `fetch` POST. Native export currently lacks the metrics needed to replace the full application exporter. Fix request scheduling separately if pursuing latency improvements. |
