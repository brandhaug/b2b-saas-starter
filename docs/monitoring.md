...
The minimum runtime inventory is two uptime monitors, three cron monitors and
metric monitors for the policies below. The backup workflow adds separate backup
and freshness cron monitors, bringing the minimum to two uptime and five cron
monitors. The external queue check adds one more cron monitor, for a total of
six. Include every deployed queue, including dead-letter queues, in its inventory. Configure a paid Sentry plan and pay-as-you-go
...
| Daily digest and retention                      | Cron `b2b-saas-starter-background-digest`, `0 8 * * *`, UTC                                                             | 5-minute grace and maximum runtime; failure threshold 1, recovery threshold 1                |
