...
- First access requires verified payment or an unexpired operator trial. Trial invoices do not establish paying history. Renewal grace starts at the first actual failed payment, and retries retain that timestamp. Grace follows that evidence rather than the reported status, because a failing renewal can still read `active`; statuses that ended access (canceled, unpaid, paused, incomplete) get none. Paid zero-due renewal invoices preserve established payment history.
