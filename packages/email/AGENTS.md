...
- One `NotificationEmail` renders every kind. `NOTIFICATION_COPY` and `NOTIFICATION_PREVIEW_PROPS` are `satisfies`-pinned to `NotificationKind`: a new DB enum kind is a type error until it has copy and preview props. Never loosen either to an index signature.
