...
| governance | [`workspace-sso-connections`](src/governance/workspace-sso-connections.AGENTS.md) | `workspaceSsoConnections` | list/describe/routing reads direct; create/update/remove via the sso-plugin binding (audit-emitting) |
...
| developer-platform | [`mcp-client-connections`](src/developer-platform/mcp-client-connections.AGENTS.md) | `oauthClient`, `oauthConsent`, `oauthRefreshToken`, `oauthAccessToken`, `workspaces` | the MCP Clients a user connected through OAuth: describe, list, revoke (audit-emitting), record grant                                                                      |
| developer-platform | [`webhook-endpoints`](src/developer-platform/webhook-endpoints.AGENTS.md)           | `webhookEndpoints`, `webhookDeliveries`, `workspaces`                                | list, create, update, delete, rotate secret (24h grace), replay, test send, deliveries (audit-emitting)                                                                    |
...
| governance         | [`account-lifecycle`](src/governance/account-lifecycle.AGENTS.md)                   | `workspaceMembers`, `workspaces`, `auditEvents`, `apiTokens`                         | deletion plan + teardown via the plugin binding and the core delete-user endpoint (audit-emitting)                                                                         |
...
2. Add `src/<context>/<capability>.ts` (Schema + Service + Seed + Live) — one file while it stays one screenful of each part.
   **Split it once the file passes ~300 lines, or as soon as a sibling needs one part of it without the rest.** The seam is always the same three modules, no barrel and no re-export shim (invariant 5), so consumers import the specific one:
   ...
   Already split: `webhook-endpoints`, `api-token-registry`, `workspace-invitations`, `billing`, `notification-feed`, `workspace-export` (which also has two pure siblings: `workspace-export-archive.ts`, the ZIP builder, and `workspace-export-snapshot.ts`, the cross-capability read the background worker and the Seed adapter share). Everything else is one file, and should stay one until it earns the split.
