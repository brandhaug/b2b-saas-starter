...
| governance         | [`plan-entitlements`](src/governance/plan-entitlements.AGENTS.md)           | `apiTokens`, `webhookEndpoints`, `workspaceMembers`, `workspaces` | `checkLimit(resource)` + `usage`; enforced at the api-token mint, webhook create, and invitation send seams |
