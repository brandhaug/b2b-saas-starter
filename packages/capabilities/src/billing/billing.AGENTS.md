...
- `billing.ts` — the contract: `Billing` service + `BillingInterface`, `CheckoutInput` / `PortalSession`, `SeatSyncResult`, `ApplySubscriptionEventInput`, the audit-metadata helpers, and the provider-event reduction both adapters share (`nextSubscriptionState` + `seatQuantityMoved` over `SubscriptionState`) — the "what does an event leave on the row" rule exists once, here, not mirrored per adapter. No adapters live here.
