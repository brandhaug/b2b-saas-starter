...
- After ownership validation, invalidate conversation access before each plugin write. Failed invalidation leaves membership unchanged so the request can retry. A later plugin refusal can still interrupt answers and delete prepared personal exports; accept this conservative cost. Same-role requests remain no-ops. D1 membership triggers also advance policy/run revisions on committed writes, so revision counts can differ from Seed; both adapters preserve monotonic invalidation. Security evidence describes completed membership writes and follows the mutation.
