...
- Mutations (`create`, `revoke`) run their row write and the audit insert as **one atomic D1 batch** (via the effectful `AuditEventLog.prepareRecord` + `batch` from `@b2b-saas-starter/db`). All methods can additionally fail with `CapabilityUnavailable` (503) when D1 is unreachable.
