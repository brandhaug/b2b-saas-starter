...
- `publishWebhookEventWith(publisher, { eventType, payload })` — the best-effort composition the mutating capabilities use: a failed publish annotates the wide event (`webhookPublish: 'failed'`) but never fails the mutation. Taking the publisher as an argument keeps `WebhookPublisher` out of every capability's interface; fan-out is implementation detail below the seam, identical for REST, MCP, and the web session surface.
