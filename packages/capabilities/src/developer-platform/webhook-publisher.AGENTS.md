...
- `WebhookPublisher.requeue(message)` — manual-redelivery enqueue for exactly one endpoint: sends a single message, stamping the current request's `traceparent`. No fan-out and no subscription check — the caller (`redeliverWebhookDelivery` in `webhook-endpoints.ts`) has already verified the delivery row. Same no-op-without-binding posture as `publish`.
