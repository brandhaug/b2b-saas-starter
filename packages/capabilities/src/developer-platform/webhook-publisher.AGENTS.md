...
- `WebhookQueueMessage` — `{ endpointId, eventType, payload }`, exported as both the `effect/Schema` struct and its `Type`. This package owns the wire shape; the queue consumer in `apps/background` imports it from here instead of maintaining a parallel type.
