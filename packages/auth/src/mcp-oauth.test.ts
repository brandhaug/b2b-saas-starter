...
// Both take the plugin's own callback bags, which is what lets the options
// object assign them directly.
...
describe('mcpWorkspaceNeedsSelection', () => {
  ...
  it('redirects every request that does not vouch for the workspace on the session', () => {
    expect(mcpWorkspaceNeedsSelection({ headers: new Headers(), session })).toBe(true)
    ...
    expect(
      mcpWorkspaceNeedsSelection({
        headers: new Headers({ [MCP_WORKSPACE_SELECTED_HEADER]: 'wrk_b' }),
        session
      })
    ).toBe(true)
    expect(
      mcpWorkspaceNeedsSelection({
        headers: new Headers({ [MCP_WORKSPACE_SELECTED_HEADER]: 'wrk_a' }),
        session: { id: 'ses_1', activeOrganizationId: null }
      })
    ).toBe(true)
  })
...
describe('mcpWorkspaceReferenceId', () => {
  it('is the picked workspace', () => {
    expect(
      mcpWorkspaceReferenceId({
        session: { id: 'ses_1', activeOrganizationId: 'wrk_a' }
      })
    ).toBe('wrk_a')
  })
  ...
  it('refuses to consent without a pick', () => {
    ...
    expect(() => mcpWorkspaceReferenceId({ session: { id: 'ses_1' } })).toThrow(
      APIError
    )
    expect(() => mcpWorkspaceReferenceId({ session: undefined })).toThrow(APIError)
  })
})
