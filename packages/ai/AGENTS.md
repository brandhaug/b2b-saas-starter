...
- Callers pass currently authorized completed exchanges to `prepare`, with one successful answer per question. Incomplete answer text and unanswered questions stay in saved history. Up to three recent failure observations carry only question/attempt IDs and fixed safe categories, never partial answers, task evidence or raw diagnostics. Historical task evidence retains task/source IDs and observation time.
...
- Stateless `ask` validates input, bounds inference to 60 seconds and rejects provider-reported incomplete output. Absent completion metadata remains unknown; text alone cannot establish truncation. Nonstreaming adapters request 4096 output tokens by default. Shared action/evidence instructions apply even without task evidence. OpenAI cancellation spans response consumption; Workers binding cancellation cannot guarantee provider work stops.
