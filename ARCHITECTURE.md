...
- **Workspace SSO (ADRs 0069 and 0077):** owners configure OIDC or SAML connections, verify the exact email domain, and complete a real IdP login before activation. Required SSO is checked on protected workspace requests, including existing sessions, rather than blocking account-wide password login. Session-bound proofs name the current connection generation and expire independently of session refresh. Existing members and invitees are admitted by default; optional verified-domain auto-join grants only `member`. Protocol writes use the plugin binding; starter-owned policy and replacement state use audited D1 batches. DNS failures retain a fixed seven-day grace period, then stop new SSO authentication while the workspace requirement remains enforced. Operator recovery is time-limited, independently authenticated, and restricted to SSO repair.
