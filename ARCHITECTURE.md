...
- **Wrangler configs:** [`apps/{api,background,web}/wrangler.jsonc`](./apps) mirror the alchemy bindings for `wrangler dev` and `wrangler d1 migrations apply`. Alchemy is the source of truth for production — when in doubt, change `alchemy.run.ts` and mirror to the relevant `wrangler.jsonc`. The `database_id: "placeholder"` literal is replaced by `wrangler ... --remote` at command time.
- **Rate limiting:** Cloudflare's native `RateLimit` binding (one per bucket, declared in alchemy + wrangler). The runtime layer ([`apps/api/src/rate-limit.ts`](./apps/api/src/rate-limit.ts), [`apps/web/src/lib/rate-limit.ts`](./apps/web/src/lib/rate-limit.ts)) calls `binding.limit({ key })` when the binding is present and falls back to a per-isolate `HashMap` brake when it isn't (local dev/tests). Distributed limits are eventually-consistent across regions — swap to a Durable Object if you need strong consistency.
...
| `GITHUB_CLIENT_ID` / `GITHUB_CLIENT_SECRET`   | optional    | web             | OAuth disabled                                          |
| `STRIPE_SECRET_KEY` / `STRIPE_WEBHOOK_SECRET` | optional    | background, api | billing module inactive                                 |
| `SENTRY_DSN`, `POSTHOG_KEY`, `POSTHOG_HOST`   | optional    | all             | observability module inactive                           |
| `CLOUDFLARE_EMAIL_FROM`                       | optional    | background      | email module inactive                                   |
| `TURNSTILE_SITE_KEY` / `TURNSTILE_SECRET_KEY` | optional    | web             | captcha disabled                                        |
| `WORKERS_AI_ENABLED`, `OPENAI_API_KEY`        | optional    | api, background | AI module inactive                                      |

Secrets are wrapped in `effect/Redacted` in [`alchemy.run.ts`](./alchemy.run.ts) so they never appear in logs or stack traces. Optional providers are validated module-aware in [`packages/env/src/server.ts`](./packages/env/src/server.ts) — missing config degrades the module to inactive instead of failing startup.
