...
const confirmationHtml = (
  ...
  const partyAppointments = confirmation.appointments
    .map(
      (appointment) =>
        `<li><strong>${escapeHtml(appointment.snapshot.customerDetails.name)}</strong><span>${escapeHtml(appointment.snapshot.assignedProvider.displayName)} · ${formatBookingDate(confirmation.locale, appointment.startsAt, appointment.snapshot.merchantTimezone)} ${formatBookingTime(confirmation.locale, appointment.startsAt, appointment.snapshot.merchantTimezone)}</span></li>`
    )
    .join('')
  const party =
