...
describe('Booking selection flow', () => {
  ...
  it('uses the legacy service-card contract and waits through its selected transition before add-ons', async () => {
    ...
    expect(screen.getByText('Anything you wish to add?')).toBeTruthy()
    const confirmedIcon = screen.getByTestId('icon:confirmed')
    expect(confirmedIcon.tagName).toBe('DIV')
    const confirmedCheck = confirmedIcon.querySelector('svg[viewBox="0 0 11 8"]')
    const confirmedClose = confirmedIcon.querySelector('svg[viewBox="0 0 10 10"]')
    expect(confirmedCheck).toBeTruthy()
    expect(confirmedClose).toBeTruthy()
    const initialCheckClass = confirmedCheck?.getAttribute('class')
    const initialCloseClass = confirmedClose?.getAttribute('class')
    fireEvent.mouseEnter(screen.getByTestId('service:svc_cut'))
    expect(confirmedCheck?.getAttribute('class')).not.toBe(initialCheckClass)
    expect(confirmedClose?.getAttribute('class')).not.toBe(initialCloseClass)
    vi.useRealTimers()
  })
