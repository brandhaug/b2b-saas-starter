...
- Two credentials open `/mcp` (ADR 0068): a JWT goes to the OAuth verifier, anything else to the API Token path. A token authorizes as its scopes, an OAuth caller as the Member re-resolved per call, so removals and role changes apply at once. Both draw the `mcp` bucket and reject via `guardFailureResponse`.
...
- `POST /mcp` has no route-level permission check by design: every minted credential clears `mcp:read`, so a gate could never deny. Enforcement is per tool.
