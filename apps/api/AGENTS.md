...
- **Invitations are send-only here.** `invitations.send` emails and publishes a webhook event, and persists nothing. Every invitation endpoint on Better Auth's `organization` plugin is `requireHeaders: true`, and `createInvitation` additionally requires the caller to be a workspace member holding `invitation:create` — this worker authenticates with a bearer token and has no session to offer. The persisted flow lives in `apps/web` (see [`workspace-invitations`](../../packages/capabilities/src/governance/workspace-invitations.AGENTS.md)); issue #64 owns how, or whether, the worker gets a session. The link this endpoint emails (`/invitations/accept?workspace=<slug>`) carries no invitation id and therefore still resolves to the "cannot be used" notice.
