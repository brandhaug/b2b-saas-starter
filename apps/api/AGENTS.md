...
- `makeOAuthTokenVerifierLayer` throws at layer build for a non-`https:` JWKS URL in production; layer construction has no Effect channel, so the throw is the gate, refusing at startup as `apps/web`'s env gate does. Unset OAuth env leaves the verifier inactive.
