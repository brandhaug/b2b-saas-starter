 ...
 * loader + composition and the card lives beside the other auth components.
 * Everything user-facing about signing in with a password, a link, a social
 * provider, or a passkey is here; failure copy comes from the shared code
 * table (`lib/auth-error-copy.ts`) — a raw `error.message` never renders.
 */
 ...
 * probe rather than an effect Schema: this page is statically imported by the
...
/**
 * The sign-in screen: the Local Auth Paths as one card's modes — password
 * (default) and the emailed link — plus the passkey block and the social
 * providers the loader resolved. Rendered by the route from its search
 * params and loader values, and from tests with plain props, no route tree
 * and no mocked router.
 */
