 ...
 * the email one-time-code endpoints, and the second-factor check — sending a
 * code or a link is an email-sending primitive, and verifying a code, a link
 * or a TOTP is a guessable-credential check, so all sit in the same bucket as
 * a password guess (ADR 0030). The password-reset request joins them in
 * either spelling: the link flow and its `/email-otp` sibling are the same
 * mail-an-arbitrary-address primitive.
 *
 * The twoFactor plugin's own `otp` method is off in `packages/auth` (TOTP
 * only), so there is no `/two-factor/send-otp` to bucket.
 */
const AUTH_SIGN_IN_SUFFIXES = [
  ...
  // The second factor is a six-digit guessable secret; the hop that checks it
  // is as brute-forceable as the password hop that precedes it.
