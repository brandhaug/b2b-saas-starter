 ...
 * `@b2b-saas-starter/capabilities` declares. `updateMemberRole` and
 * `removeMember` are `requireHeaders: true` (ADR 0051 keeps membership writes
 * out of the API worker for that reason); the add-member route is trusted and
 * headerless. See `./plugin-call.ts`.
 */
