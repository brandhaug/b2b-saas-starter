...
Owners can export a workspace as a ZIP from workspace settings (ADR 0054). The archive is built by the background worker and stored in an R2 bucket for seven days; downloads go through a signed link on the API worker.
