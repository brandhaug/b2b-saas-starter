...
CAPTCHA env gate. No Turnstile widget is rendered anywhere in the app yet — the variables are validated by the schema, so adding a widget later needs no schema change.
...
**Verify:** set both variables in `.env` and restart — activation is a schema-level check until a widget ships.
...
The env gate for billing. Checkout, the customer portal, and the inbound webhook route are planned and not yet implemented — see the Stripe doc in the Integration surfaces section for the planned scope.
...
There is no billing UI yet.

## Sentry and PostHog (observability)

Error monitoring and product analytics. No SDK is initialized in the Workers yet; the env gate exists so init can land without a schema change. Live telemetry today goes through `OTEL_EXPORTER_OTLP_ENDPOINT` instead.
