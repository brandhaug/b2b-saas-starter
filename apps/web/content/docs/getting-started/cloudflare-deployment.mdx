...
`pnpm run destroy --confirm-target="$CLOUDFLARE_ACCOUNT_ID/prod"` tears the entire stack down after checking the exact account and stage. Use Cloudflare's Worker version controls for a code rollback after confirming the previous release is compatible with the current schema and bindings. A database restore follows the recovery runbook and remains an operator action.
