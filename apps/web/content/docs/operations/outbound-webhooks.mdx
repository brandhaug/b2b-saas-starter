...
Each request body is `{"deliveryId": "whd_…", "eventType": "…", "payload": …}`, where `deliveryId` equals the persisted delivery row id (so you can deduplicate redeliveries), and carries these headers:
