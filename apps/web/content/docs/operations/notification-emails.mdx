...
Durable delivery records track attempts and provider acceptance. Temporary or
ambiguous notification sends can retry within 24 hours of creation. Before each
attempt, the consumer rechecks relevance, recipient access, and preferences.
Once the provider accepts a message, the app does not submit it again.
...
Failed digests retry every fifteen minutes against the same 08:00 UTC window,
with no new send after 14:00 UTC. Each pass rereads unread notifications and
preferences; stable per-user delivery IDs prevent resending accepted digests.
Queue settings and schedules live in `infra/bindings.ts`.

Cloudflare delivery events update sanitized history. Provider acceptance and
inbox delivery are separate states. The application retains no rendered email
bodies, OTPs, or secret links in those records.
...
3. With the background worker running, trigger its scheduled test endpoint: `curl "http://localhost:8788/__scheduled?cron=0+8+*+*+*"`. The digest email renders to the worker log.
