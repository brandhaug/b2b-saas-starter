...
Read tools include `get_workspace_overview`, `list_members`,
`list_notifications`, `list_api_tokens`, `list_webhooks`,
`list_webhook_deliveries`, `list_webhook_delivery_attempts`, and `list_audit_events`, with the
...
A connected client can separately request the assistant resource and `assistant:read` / `assistant:write` scopes. Consent lists those resources explicitly. MCP-audience tokens remain invalid on private conversation REST, and assistant-audience tokens remain invalid on MCP. Revoking the client connection revokes its granted resources.

## Connect from a script

Scripts keep using API Tokens: `Authorization: Bearer bsk_live_…` on `POST /mcp`, as on workspace REST routes. Private Assistant Conversation REST requires its separate member-OAuth resource instead. See [API tokens](/docs/capability-interfaces/api-tokens). A seeded local D1 accepts the documented seed token.
