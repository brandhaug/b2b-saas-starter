...
- `src/lib/capabilities.ts` reads bindings from `cloudflare:workers` (same as `server-context.ts`): with a real `DB` binding the Live D1 layer activates; under the local dev shim (`cloudflare-workers-shim.ts`, `DB: undefined`) the in-memory Seed layer keeps the app provider-light. Never hardcode an empty env.
