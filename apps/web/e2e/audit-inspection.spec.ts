...
test('event links preserve filters, keyboard focus, and back/forward history', async ({
  ...
  await expect(link).toHaveAttribute('href', /event=aud_token/)
  const eventUrl = new URL((await link.getAttribute('href')) ?? '', filteredUrl)
  expect(eventUrl.searchParams.get('view')).toBe(serializedView)
