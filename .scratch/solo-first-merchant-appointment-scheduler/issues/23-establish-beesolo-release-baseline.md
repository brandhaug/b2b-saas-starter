...
Verification passed after the final correction pass: the executable release baseline now runs 93 tests across canonical fixture, live Booking, HTTP, Public Site dispatch, Customer association, and typed API callback seams. All 25 monorepo typecheck tasks pass. The candidate command deliberately exits `1` on current planned parity, deferred payment and Platform API ingress, active starter identity, public Platform API documentation, typed Team or multi-Provider launch surfaces, and observable Team, Provider navigation, or Provider-choice UI. Customer Directory convergence updates only its association field; Booking owns initialization of immutable origin and customer-note facts. Scheduling and Booking now fail closed when canonical schedule, Appointment, or confirmation-access facts cross their owning fixture boundaries. The full monorepo test run remains blocked outside this ticket by local D1 integration setup failures (`EADDRNOTAVAIL`) and a concurrent Solo-entitlement migration-test failure.
