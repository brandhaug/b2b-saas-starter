...
The deterministic fixture assertion now lives in Merchant Catalog and runs during seed and candidate creation. It enforces the Solo Plan, one active default Owner-Provider, Merchant boundaries, explicit active-Service eligibility, valid non-overlapping Appointment intervals, and non-orphaned Confirmation access; the executable evidence suite binds the existing live conflict, snapshot, outbox, network-fresh HTTP, and Public Site dispatch tests.

Verification passed: the 60-test executable release baseline, 20 typed API callback tests, focused release-policy and Merchant fixture tests, scoped `oxfmt`, and `git diff --check`. The candidate command deliberately exits `1` on current planned parity, deferred payment and Platform API ingress, active starter identity, public Platform API documentation, Team behavior, Provider navigation, and Provider-choice code. The final repeated Standards/Spec review found no remaining issue-23 gaps.
