...
- [Expand D1 for beesolo](issues/24-expand-d1-for-beesolo.md) — Resolved after exhaustive zero-finding Standards and Spec reviews with forward-only foundations, restore-external Privacy Action Ledger authority, exact Solo preflight and tenant guards, truthful resumable recovery, revision-bound privacy preflights, and collection-safe revisioned Appointment snapshots while old Workers remain compatible.
- [Deliver Customer Directory Foundations](issues/28-deliver-customer-directory-foundations.md) — Connect durable Merchant-scoped Customer Records to the authenticated directory and Appointment composer seams, with revisioned Owner operations, attributed provenance, conservative duplicate recovery, and immutable Appointment snapshots.
